    ...
    public class RedYellowBlue : BaseColorModel, IEquatable<RedYellowBlue>
    {
        ...
        public static RedYellowBlue ParseSpecifier(string specifier)
        {
            if (!IsSpecifierValid(specifier))
                throw new ColorException(LanguageTools.GetLocalized("COLORIMETRY_MODEL_EXCEPTION_PARSEINVALIDRYBSPECIFIER").FormatString(specifier) + ": ryb:<red>;<yellow>;<blue>");
            ...
            if (specifierArray.Length == 3)
            {
                ...
                int r = Convert.ToInt32(specifierArray[0]);
                if (r < 0 || r > 255)
                    throw new ColorException(LanguageTools.GetLocalized("COLORIMETRY_MODEL_EXCEPTION_PARSERYBRLEVEL") + $" {r}");
                int y = Convert.ToInt32(specifierArray[1]);
                if (y < 0 || y > 255)
                    throw new ColorException(LanguageTools.GetLocalized("COLORIMETRY_MODEL_EXCEPTION_PARSERYBYLEVEL") + $" {y}");
                int b = Convert.ToInt32(specifierArray[2]);
                if (b < 0 || b > 255)
                    throw new ColorException(LanguageTools.GetLocalized("COLORIMETRY_MODEL_EXCEPTION_PARSERYBBLEVEL") + $" {b}");
                ...
                throw new ColorException(LanguageTools.GetLocalized("COLORIMETRY_MODEL_EXCEPTION_PARSEINVALIDRYBSPECIFIEREXCEED").FormatString(specifier) + ": ryb:<red>;<yellow>;<blue>");
        }
